...
describe(SafeHtmlPipe.name, () => {
	describe(SafeHtmlPipe.prototype.transform.name, () => {
		...
		it.each<{html: string; expected: SafeHtml}>([
			...
			{
				html: 'bla <strong> bla </strong>',
				expected: {changingThisBreaksApplicationSecurity: 'bla <strong> bla </strong>'},
			},
			{
				html: 'bla <script> bla </script>',
				expected: {changingThisBreaksApplicationSecurity: 'bla <script> bla </script>'},
			},
