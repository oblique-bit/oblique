...
@Component({
	...
	template: `<app-tabs [idPrefix]="componentId" [id]="componentId | id: ['tabs']">
		<app-tab
			...
			[idPrefix]="componentId | id: ['tab', 1]"
			[id]="componentId | id: ['tab', 1]"
		>
		...
		<app-tab
			...
			[idPrefix]="componentId | id: ['tab', 2]"
			[id]="componentId | id: ['tab', 2]"
		>
...
describe(TabsComponent.name, () => {
	...
	it('should have 1st tab be active', () => {
		expect(
			getClassForDebugElementById<TabsWrapperComponent>(
				fixture,
	...
	it.each<{id: number}>([{id: 1}, {id: 2}])('should display tab with id: $id', ({id}) => {
		expect(
			getDebugElementById<TabsWrapperComponent>(fixture, idPipe.transform(component.componentId, ['tab', id]))
		).toBeTruthy();
	});
