...
class Tarball extends StaticScript {
	...
	private static buildTarball(): string {
		executeCommandWithLog('npm run build', 'Build Ob-Tour');
		Log.info('Pack Ob-Tour');
		const data = getResultFromCommand('npm pack', {cwd: Files.buildOSSafePath('../../dist/ob-tour')});
		return /oblique-ob-tour-\d+\.\d+\.\d+(?:-(?:alpha|beta|rc)\.\d+)?\.tgz/.exec(data)[0];
	}
