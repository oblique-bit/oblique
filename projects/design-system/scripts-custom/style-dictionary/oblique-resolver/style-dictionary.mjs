	...
	return modes.map(
		mode =>
			new StyleDictionary({
				...
				// Local adaptation: the dev's original throws on any broken
				// reference. tokens-dev currently has dangling refs (undefined
				// ob.s.shadow.* referenced by 05_html/button/03_shadows.json).
				// 'console' downgrades those to warnings so the resolver still
				// emits CSS for every token that does resolve.
				log: {
