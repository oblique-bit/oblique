...
2. **In-Figma side** (`PLUGIN_CODE`):
   - Discovers all required components by name (`_docs/color-variables/*` + `_docs/shared/group_header`), caches them locally per Figma file to skip the slow walk on subsequent runs.
...
- **Slow first run** — discovery cache is cold. First run walks the document; subsequent runs hydrate from the local cache.
