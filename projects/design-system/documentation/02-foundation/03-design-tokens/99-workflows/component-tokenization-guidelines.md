...
- **UI Scale:** `ob.s.dimension.dynamic.ui_scale.*` (if you need spacing to change on mode switch) or `ob.s.dimension.static.ui_scale.*` (for stable spacing that does not react on mode switch)
