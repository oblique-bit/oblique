...
The primitive layer stores one `px` scale only. `rem` is computed at the **semantic** layer from the `px` value (`{...px} / 16 * 1rem`), not stored as a separate primitive — see [Dimension Tokens](../02-categories/02-dimension-tokens.md) for the full semantic-layer breakdown.
