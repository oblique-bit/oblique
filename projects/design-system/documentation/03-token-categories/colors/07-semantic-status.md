...
- **Status icon** - Visual symbols for enhanced recognition, covered in the icon documentation
...
**Semantic Aura Rule:**
If a shade is directly consumed by a status token, the **next 2 shades in each direction** on that color's scale are also reserved, so a decorative use never sits one step away from a status color and gets mistaken for it. A reserved hue can consume most of its own scale (see the Critical (Red) reference table below), so always check the full reference table for a hue before treating any of its shades as free for decorative use.
...
- **Symmetrical light/dark behavior.** In light mode, `fg.contrast_highest` is the darkest neutral and `bg.contrast_highest` is white. In dark mode, the values swap. The concept — "give me the maximum" — stays the same regardless of mode.
...
#### Foreground Colors
| Contrast Level | Light Mode | Dark Mode | Primitive Token (Light) | Primitive Token (Dark) | Usage |
|----------------|-------------|------------|-------------------------|------------------------|-------|
...
> **Proposed, not yet implemented.** Pending, Confirmed, Progress, Scheduled, Waiting, and Closed are not present in the shipped token source (`src/lib/themes/`), which currently defines only `info`, `resolved`, `critical`, `attention`, and `fatal` as status tokens. The values below are illustrative proposals for future workflow-status tokens, not documentation of tokens that exist today.
...
| Contrast Level | Light Mode | Dark Mode | Primitive Token (Light) | Primitive Token (Dark) | Usage |
|----------------|-------------|------------|-------------------------|------------------------|-------|
| `contrast_high` | `#eef2ff` | `#4338ca` | `ob.p.color.indigo.50` | `ob.p.color.indigo.700` | *Not used in current components* |
...
The remaining flexible statuses below follow the same proposed structure as Pending, Confirmed, and Progress.
...
3. **Test mode switching** - Ensure inversity variations work correctly
...
*For related documentation, see [Free Colors](08-semantic-free.md), Neutral Colors and Interaction Colors*
