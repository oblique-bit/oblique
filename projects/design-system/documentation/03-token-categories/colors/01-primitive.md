...
Every primitive scale is now claimed by a semantic tier — there is no "safe for any decorative purpose" stage left. An application-specific accent color that carries no fixed meaning goes through `ob.s.color.free.*` (see [Semantic Free Colors](08-semantic-free.md)), not a hand-picked primitive.

**For guidance on decorative vs semantic color usage, see [Color System Overview](00-overview.md).**

**Usage Recommendations (in order of preference):**

1. **Need an accent color with no fixed meaning**: Use `ob.s.color.free.*` — see [Primitive Color Consumption](02-primitive-consumption.md)
