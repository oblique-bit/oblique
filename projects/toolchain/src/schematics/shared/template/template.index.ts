...
export function templateIndex(): Rule {
	return (tree: Tree, context: SchematicContext) => {
		return chain([createFromTemplate(path.join(__dirname, 'templates'))])(tree, context);
	};
}
export function templateOptions(options: ObSchemaOptions): Rule {
	return (tree: Tree, context: SchematicContext) => {
		return chain([createFromTemplate(path.join(__dirname, 'templates'), [template(options)])])(tree, context);
	};
}
