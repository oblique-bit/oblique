...
class Release extends StaticScript {
	static perform(version: string, issue: string): void {
		...
		Release.updatePublicode(version);
	...
	private static updatePublicode(version: string): void {
		// ignores preversions
		if (/^\d+\.\d+\.\d+\$/u.test(version)) {
			Log.info('Update publicode release version and date');
			const today = new Date().toISOString().split('T')[0];
			Files.overwrite('publicode.yml', content =>
				content
